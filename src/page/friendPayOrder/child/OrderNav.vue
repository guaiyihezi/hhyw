...
export default {
  ...
  methods: {
    ...
    setOrderNavActive(index) {
      ...
      this.getOrderList()
      if (index == '') {
        sendBuryingPointInfo({ click_position: 'profile_friendPay_all' })
      } else if (index == ENUM.ORDER_STATUS.FINISHED) {
        sendBuryingPointInfo({ click_position: 'profile_friendPay_finish' })
      } else if (index == ENUM.ORDER_STATUS.CANCELED) {
        sendBuryingPointInfo({ click_position: 'profile_friendPay_cancel' })
      }
    },
    ...
    getOrderList(ispush) {
      ...
      orderFriendPayList(data.page, data.size, data.status).then(res => {
        if (ispush) {
