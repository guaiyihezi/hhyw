      ...
      <div class="hb-discount-title">
        ...
        <span class="hb-price-desc">
          ...
          <!-- <span class="hb-price-desc-txt">
            {{
              `${utils.formatFloat(detailInfo.origin_price.HB_SHOW)}权益+￥${utils.formatFloat(
          ...
          <div class="quanyi-wrapper">
            ...
            <div>
              <label class="debt-style">{{ `权益抵扣￥${utils.formatFloat(detailInfo.origin_price.HB_SHOW)}` }}</label>
            </div>
          </div>
        </span>
      </div>
    ...
    <div class="product-live" v-if="productSharer.length > 0">
      ...
      <div class="product-live-body">
        <template v-if="index < 2">
          <div class="product-live-item" v-for="(item, index) in productSharer" :key="index">
  ...
  methods: {
    ...
    showMoreSharer() {
      ...
      if (this.productSharer.length == 10) {
        msg += '<p class="product-live-p">仅显示最近10个换换客</p></div>'
      }
      this.$messagebox({
        title: '这些换换客正在推荐该商品',
        message: msg,
