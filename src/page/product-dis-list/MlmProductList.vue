<template>
  <div class="mlm-prod-list">
    ...
    >
      <!-- 无限加载滚动列表 -->
      <div class="clearfix">
        <div class="product-group" v-for="(item, index) in dataListArr" v-bind:key="index">
          <product-body :item="productList[index * 2]" :productId="productList[index * 2].id"> </product-body>
          <template v-if="productList[index * 2 + 1]">
            <product-body :item="productList[index * 2 + 1]" :productId="productList[index * 2 + 1].id"> </product-body>
          </template>
        </div>
