...
<template>
  <div class="product-detail-wrapper">
    ...
    <template v-if="detailInfo.id && !notAvailable">
      ...
      <!-- 提示秒杀banner -->
      <seckill-banner
        v-if="detailInfo.in_secbuy && detailInfo.in_secbuy.id"
        :secbuyObj="detailInfo.in_secbuy"
      ></seckill-banner>
      <!-- 优惠券popup -->
    ...
    */
    getDetail() {
      ...
      productGet(this.productId, this.$route.query.modproductkey)
        .then(
          res => {
            if (res.sale_type == ENUM.GOODSSALETYPE.AUCTION) {
              this.$router.replace({ name: 'auctionDetail', query: { id: res.special_id } })
            } else {
              this.saveInfo(res)
              this.saveSaleFlag()
