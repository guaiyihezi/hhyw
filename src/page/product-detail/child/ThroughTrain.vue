      ...
      <template v-if="troughInfo.train_sn">
        <div class="through-popup-wrapper">
          ...
          <template v-if="isHHApp">
            <button @click="share" class="share-bth"><span>分享好友</span></button>
            <p class="dialog-slogan">便宜不能独占，与好友一起省钱</p>
          </template>
        </div>
      </template>
      <template v-else>
...
.dialog-slogan{
  @include sc(10px, #D2A06B, center center);
    font-weight:400;
    margin-top: 3px;
  }
}
