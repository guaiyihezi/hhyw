...
<template>
  <div class="swipe-wrapper ui-common-swiper" v-if="detailInfo">
    ...
    >
      <template v-if="detailInfo.photos && detailInfo.photos.length > 0">
        <mt-swipe-item v-for="(item, index) in detailInfo.photos" v-bind:key="index" ref="media">
          ...
          <img
            class="tags-icon"
            v-if="index == 0 && detailInfo.tags_info && detailInfo.tags_info.length"
            :src="detailInfo.tags_info[0].info_img"
          />
        </mt-swipe-item>
      </template>
    ...
    <div class="cat-fq-banner" v-if="detailInfo.instalment && detailInfo.instalment.length > 0">
      <span>权益抵扣租金{{ Math.round((100 * detailInfo.money_line) / detailInfo.current_price) }}%</span>
    </div>
  </div>
</template>
