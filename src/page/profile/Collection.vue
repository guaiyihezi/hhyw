  ...
  <div class="container">
    ...
    <div class="body" v-infinite-scroll="getMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
      ...
      <div v-if="!isMore && collectionList.length <= 0" class="order-air">
        <img src="../../assets/image/change-icon/favorite_empty.png" />
        <p>暂无商品</p>
        <gk-button class="button" type="primary-secondary" v-on:click="goVisit">
          <label>随便逛逛</label>
        </gk-button>
      </div>
    </div>
...
export default {
  data() {
    return {
      collectionList: [],
      orderListParams: { page: 0, size: 10 },
      loading: false,
      isMore: true
    }
  },
  ...
  methods: {
    ...
    orderCollection(isFirst) {
      ...
      productLikedList(data2.page, data2.size).then(
        res => {
          this.loading = false
