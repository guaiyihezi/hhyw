...
<template>
  <div class="product-detail-wrapper">
    ...
    <template v-if="detailInfo.id && !notAvailable">
      ...
      <!-- 权益说明popup -->
      <promotion-popup v-if="promoPopstatus" :promoPopstatus="promoPopstatus"></promotion-popup>
      <!-- 权益说明popup -->
...
import DetailBody from '../product-detail/DetailBody'
...
import PopupService from '../product-detail/child/PopupService'
import PromotionPopup from '../product-detail/child/PromotionPopup'
import BackTop from '../product-detail/child/DetailBackTop'
// import { productGet } from '../../api/product'
import { secProductDetail } from '../../api/promotion'
...
export default {
  ...
  created() {
    ...
    if (!this.isPromotion) this.getCouponList()
  ...
  methods: {
    ...
    */
    getDetail() {
      this.$indicator.open()
      secProductDetail(this.productId)
        .then(
          res => {
            if (res.secbuy && res.secbuy.type == 0) {
              this.$router.replace({ name: 'SeckillProduct', query: { id: this.productId } })
            }
            // 原始商品id
            this.setCurrentProductId(res.id)
