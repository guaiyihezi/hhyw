  ...
  <div class="order-body" v-if="orderDetail.id">
    <div class="order-body-top">
      ...
      <div v-if="isInternal">
        ...
        <div class="image" v-if="orderDetail.order_status == 1">
          <div class="order-countdown">
            ...
            <span class="ms2" v-if="orderDetail.status == 1">佣金及权益变现冻结中 买家确认收货7天后可提现</span>
            <span class="ms2" v-if="orderDetail.status == 4">佣金及权益变现可提现</span>
          </div>
        </div>
        ...
        <div class="image" v-if="orderDetail.order_status == 3">
          <div class="order-countdown">
            ...
            <span class="ms2">该订单已过期 权益无法变现</span>
          </div>
        </div>
      </div>
      <!-- 外部换换客 -->
      ...
      <div class="container radius">
        ...
        >
          <template v-if="index <= orderIndex">
            <img
              class="photo"
      ...
      <div class="desc" v-if="isInternal">
        <div class="price-info">
          <p>权益变现结算<img src="../../../assets/image/hh-icon/mlm/icon-overdue.png" v-if="InvalidStatus2" /></p>
          <checkout-desc
            class="desc-item"
          ...
          <div class="button-content" v-if="orderDetail.order_status == 0">
            <button @click="payment(orderDetail.id)">权益变现</button>
          </div>
...
export default {
  ...
  computed: {
    ...mapState({
      isInternal: state => state.mlm.isInternal //是否为内部换换客
    }),
    //分销总额
    getTotal() {
    ...
    warnTxt2() {
      ...
      if (os == 1 && s == 1) {
        ...
        warn_txt = '订单已取消 代付的权益 变现的金额将会退回'
      }
      return warn_txt
    },
    // 已失效图标
