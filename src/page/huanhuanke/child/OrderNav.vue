    ...
    >
        ...
        <div class="list" v-for="item in orderList" :key="item.id">
          ...
          <div class="order-list-opratio" v-if="item.status == 0 && item.change_rebate && isInternal">
            <div class="btn" v-if="item.status < 5 && item.order_status != 4">
              <button class="buttonright" v-on:click="payment(item)">权益变现</button>
            </div>
          </div>
          -->
        </div>
      ...
      <div v-if="orderList.length <= 0 && !isMore">
        <div class="order-air">
          ...
          <button class="button" v-on:click="goVisit" v-stat="{ id: 'profile_lookaround' }">
            随便逛逛
          </button>
        </div>
  ...
  methods: {
    ...
    setOrderNavActive(index) {
      ...
      if (index == ENUM.ORDER_STATUS.ALL) {
        sendBuryingPointInfo({ click_position: 'profile_fenxiao_all' })
      } else if (index == ENUM.ORDER_STATUS.FINISHED) {
        sendBuryingPointInfo({ click_position: 'profile_fenxiao_finish' })
      } else if (index == ENUM.ORDER_STATUS.CANCELED) {
        sendBuryingPointInfo({ click_position: 'profile_fenxiao_cancel' })
      } else if (index == ENUM.ORDER_STATUS.CREATED) {
        sendBuryingPointInfo({ click_position: 'profile_fenxiao_notFinish' })
      }
    },
    ...
    getOrderList(ispush) {
      ...
      huanOrderList(data.status, data.page, data.size).then(
        res => {
          if (ispush) {
