...
export default {
  ...
  methods: {
    ...
    importProducts() {
      // 先进行弹框提示
      this.$messagebox.confirm('确定将自己的商品加入到我的小店？').then(action => {
        // 调用接口，执行同步操作
      })
    },
    continueAdd() {
    ...
    shareProdLink() {
      ...
      this.hhApp.share(
        `￥${this.mlm_share_prod.price} | 买到就省${this.mlm_share_prod.save_money}！${this.mlm_share_prod.remark ||
          this.SHARE_SAY_PLANS[0]}`,
        this.mlm_share_prod.thumb || this.utils.app_icon,
