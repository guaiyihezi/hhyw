<template>
  <div class="wrapper" :class="fromClass">
    ...
    <div class="voice_code" v-if="!isOnce">
      <p
        class="vcd2"
        @click="getVoiceCode"
        :class="{ deep: telIsOk && !isCountDown && !isVelGeting }"
        v-if="isVoiceOnce"
      >
        获取语音验证码
      </p>
      <p class="vcd2" @click="getVoiceCode" :class="{ deep: telIsOk && !isCountDown && !isVelGeting }" v-else>
        重新获取语音验证码<span v-if="isCountDown && code_type">({{ countDown }}s)</span>
      </p>
    </div>
    <div class="login">
...
export default {
  ...
  data() {
    return {
      tel: process.env.NODE_ENV === 'production' ? '' : '18866478549',
    ...
    getVel(is_voice) {
      ...
      this.getVelApi(this.tel, is_voice, this.openId ? 1 : 0)
        .then(
          ...
          error => {
            if (error.errorCode === 2033 || error.errorCode === 2034) {
              error.toastObj.close()
              this.$messagebox({
                title: '',
