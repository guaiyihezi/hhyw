<template>
  <div class="cart-list" v-if="!isEmpty">
    <div class="cart-shop-wrapper" v-for="(cartitem, cartIndex) in fmtCartList" :key="cartIndex">
      <div class="cart-shop-header" @click="goSupplier(cartitem.sn)">
        ...
        <span class="right" v-if="isShowGetCoupon(cartIndex)" @click.stop="showCouponPopup(cartIndex)">领券</span>
      </div>
      <div class="cart-list-wrapper">
        <div class="list" v-for="(item, index) in cartitem.list" :key="`cartlist${index}`">
          <div class="list-checkbox">
            <input
              ...
              :disabled="item.attr_stock == 0 || isPreSale(item.goods)"
          ...
          <div class="list-item" @click="itemClick(item)" v-stat="{ id: `shopcart_shops_${item.goods.id}` }">
            <div class="item">
              <div class="ui-image">
                ...
                <img
                  class="tag-icon"
                  v-if="item.goods.tags_info && item.goods.tags_info.length > 0"
                  :src="item.goods.tags_info[0].plate_img"
                  alt=""
                />
              </div>
              <div class="list-info">
                <div class="product-header">
                  <h3 class="product-title" v-bind:class="{ 'disabled-list': item.attr_stock == 0 && !isDeleteMode }">
                    {{ item.goods.name }}
                  </h3>
                </div>
                ...
                <div class="price-amount-wrapper">
                  <div class="info-price">
                    <p v-bind:class="{ 'disabled-list': item.attr_stock == 0 && !isDeleteMode }">
          ...
          <div class="mask-wrapper" v-if="item.attr_stock == 0 && !isDeleteMode">
            <p>商品已失效</p>
          </div>
        </div>
        <edit-goods-number
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      couponList: state => state.cart.couponList
    }),
    ...mapGetters({
  ...
  methods: {
    ...
    updateCartQuantity(id, amount) {
      cartUpdate(id, amount).then(
        res => {},
        error => {
          this.$toast(error.errorMsg || '数据更新失败')
        }
      )
    },
    /*
    ...
    showCouponPopup(cartIndex) {
      let data = {}
      data.list = [...this.couponList[cartIndex]]
      data.list.forEach(item => {
        if (!item.is_rec) item.is_rec = 1
      })
      this.saveDetailCouponInfo(data)
      this.saveCouponPopupState(true)
    },
    isShowGetCoupon(cartIndex) {
      return this.couponList[cartIndex] && this.couponList[cartIndex].length
    }
  },
  components: {
    EditGoodsNumber
