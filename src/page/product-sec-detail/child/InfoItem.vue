    ...
    <div class="info-header ui-flex">
      ...
      <template v-else>
        ...
        <label class="huan" v-if="detailInfo.secbuy.money_line > 0"
          ><span>权益抵扣￥{{ utils.formatFloat(detailInfo.secbuy.money_line) }}</span></label
        > -->
        <PriceItem 
          :cash="utils.formatFloat(detailInfo.secbuy.cash_price)" 
          :surplus="utils.formatFloat(detailInfo.secbuy.money_line)" 
          classType="price-style-miaosha"
          :discountJson="{
            discountFlag:detailInfo.secbuy.discount < 10,
            discount:detailInfo.secbuy.discount
          }"
        />
      </template>
    </div>
        ...
        <template v-if="seckillStatus === 0">
          <span class="hb-price-desc">
            ...
            <div class="quanyi-wrapper">
              <img src="../../../assets/image/hh-icon/b0-home/debt-left-angle.png" />
              <div>
                <label class="debt-style">{{ `权益抵扣￥${utils.formatFloat(detailInfo.money_line)}` }}</label>
              </div>
            </div>
          </span>
        </template>
...
.ui-detail-info {
  ...
  .info-header {
    ...
    .price {
      ...
      span {
        @include sc(23px, $priceColor);
      }
      .price-unit {
        @include sc(15px, $priceColor);
      }
    }
    .huan {
      height: 18px;
