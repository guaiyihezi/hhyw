<template>
  <div class="page-wrapper clearfix">
    ...
    <div class="btn-pay">
      <p class="btn-desc" v-if="!authStatus && platform">请在换换商城APP中，完成授权后再为好友支付权益</p>
    ...
    async getOrderDetail() {
      ...
      try {
        ...
        if (res.status != 0) {
          ...
          this.$toast('该订单已' + (res.status == 4 ? '完成' : '取消'))
          return
        }
      } catch (error) {
        this.$indicator.close()
        this.$toast('获取订单详情异常，错误信息：' + error.errorMsg)
        return
      }
    },
    needAuth() {
