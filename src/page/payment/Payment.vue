    ...
    <div class="pay-msg-wrapper">
      ...
      <div class="pay-wrapper surplus-container" v-if="!isMlmPay || isLoading">
          ...
          <template v-if="isHbUser">
            <div :class="{ 'is-friend-pay': isFriendPay || is_only_money }">
              <div class="pay-container pay-wrapper-title">
                <label class="pay-name">权益最高可抵扣</label>
              ...
              <label
                ...
                v-if="showCoinPay"
              >
                <label class="pay-name"> 权益积分 </label>
                <div class="pay-check">
              ...
              <label
                ...
                v-if="showBeanPay"
              >
                <label class="pay-name"> 浣豆 </label>
                <div class="pay-check">
                  <template v-if="isHuanBean != ENUM.HUANTYPE.BEAN">
                    <span v-if="hasBean">有浣豆可用</span>
                  </template>
                  <template v-else>
        ...
        <template v-else>
          <div>
            <div class="pay-container pay-wrapper-title">
              <label class="pay-name">权益已抵扣</label>
              <div class="wrapper-price">
            ...
            <label class="pay-container pay-wrapper-title" v-if="hhpay.token_type === ENUM.HUANTYPE.BEAN">
              <label class="pay-name"> 浣豆 </label>
              <div class="pay-check">
                <span>已支付{{ utils.formatMoney(hhpay.surplus_paid) }}浣豆</span>
                <label class="check-radio" for="bean">
                  <label class="active"></label>
                </label>
              </div>
      ...
      <template>
        ...
        <div class="popup-body" ref="popup_body">
          <div class="body-item b-line" @click="setBeanType(ENUM.BEANTYPE.MY)">
            <div class="left">
              <p class="title">
                <span>我的浣豆</span><label>(余额:{{ utils.formatMoney(hhpay.token) }})</label>
              </p>
              <p class="line">
                本单最多可用{{ utils.formatMoney(hhpay.need_token) }}个浣豆，抵扣{{
          ...
          <template v-if="companyList.length">
            <div class="body-item" @click="setBeanType(ENUM.BEANTYPE.COM)">
              <div class="left">
                <p class="title">
                  <span>代付浣豆</span><label>(可代付:{{ utils.formatMoney(selectCompany.user_partner_token) }})</label>
                </p>
                <p class="line">
                  本单最多可用{{ utils.formatMoney(selectCompany.need_surplus) }}个浣豆，抵扣{{
            ...
            <div class="select-box">
              ...
              <div class="select-body">
                <template v-if="isShowCompany">
                  <div
                    v-for="(item, companyItemIndex) in companyList"
                    class="select-body-item select-wrapper"
                    :key="companyItemIndex"
                    @click="selectCompanyItem(item)"
                  >
...
import { Header, Radio, Popup } from 'mint-ui'
...
import Vue from 'vue'
import checkPaypwd from '../../page/checkout/checkPaypwd'
...
export default {
  data() {
    return {
      ...
      alipay_html: '',
      config: this.config,
      code: 'balance', // 支付方式： 账户权益积分余额 or ...
      isHuanBean: 0, //权益积分or浣豆 1权益积分 2浣豆
      beanType: 0, //支付浣豆种类 0我的浣豆 1代付浣豆
      type: this.type,
      miss: 0,
      showflag: false,
      ...
      share_type: ENUM.SHARE_TYPE.ONLY_HB, // 好友代付类型（目前只支持纯权益积分代付）
      ENUM,
  ...
  methods: {
    ...
    getOrderMsg(orderId, parentOrderId) {
      return new Promise((resolve, reject) => {
        ...
        orderGet(orderId, this.isInstalment, parentOrderId).then(res => {
          ...
          this.isMlmPay = res.order_type == ENUM.ORDERTYPE.MLM
    ...
    setHuanBeanType(code) {
      if (this.hhpay.surplus_paid > 0) return //已支付浣豆or权益积分 选择框不能操作
    ...
    async pay() {
      ...
      if (this.hhpay.surplus_paid > 0) {
        // 已支付权益 且 使用的是浣豆支付 取hhpay.is_use
      ...
      // 若 未抵扣权益 && 有可抵扣的权益 那么必选选择一种
      if (
        0 == Number(this.hhpay.surplus_paid) &&
        0 == token_type &&
        (Number(this.hhpay.user_money) > 0 || this.hasBean)
      ) {
        return this.$toast('请选择您的权益抵扣方式')
      }

      if (this.order_need_surplus > 0) {
        // 有积分 须支付密码
        let ifPwdContinue = true
        try {
          this.pay_cert = await checkPaypwd(this.order_need_surplus)
        } catch (e) {
          ifPwdContinue = false
        }
        if (!ifPwdContinue) return
        data['pay_cert'] = this.pay_cert
      }
  ...
  .popup-body {
    ...
    .select-box {
      ...
      .select-head {
        ...
        div {
          @include wh(7px, 7px);
          background: url('../../assets/image/hh-icon/b0-home/icon-search-实心箭头-down.png') no-repeat;
