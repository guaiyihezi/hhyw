  ...
  <div class="container">
    ...
    <div class="wrapper">
      <div class="content">
        ...
        <template v-else-if="!isOnline && status == order_pay_failed">
        ...
        <template v-else-if="!isOnline && status == order_pay_succeed_friend">
        ...
        <p v-if="ifShowCouponWillRecieve" class="will-recieve-coupon">
          恭喜恭喜！订单完成后您将获得{{ coupon_count }}张优惠券。
        </p>
      </div>
      ...
      <div class="friend-pay-wrapper" v-if="!isOnline && status == order_pay_succeed_friend">
        ...
        <div class="f-p-w-content">
          <div class="f-p-w-title">需要好友代付权益</div>
      ...
      <div class="btns">
        <template v-if="!isOnline && status == order_pay_succeed && donated">
          <gk-button
            class="button"
            type="primary-share-donate"
            v-stat="{ id: 'donate_share_paySuccess' }"
        ...
        <template v-else-if="!isOnline && status == order_pay_succeed">
          <gk-button class="button" type="primary-secondary-white" v-on:click="goPaid">查看订单</gk-button>
        ...
        <template v-else-if="!isOnline && status == order_pay_succeed_mlm">
      ...
      <!-- 小额权益免密支付设置 提醒 -->
      <block-to-set-token-freepay :showFromProp="isShowWithoutPay"></block-to-set-token-freepay>
...
export default {
  data() {
    return {
      ...
      red_cash_back: '', // 红包返现
      coupon_count: 0, // 将获得优惠券张数
      donated: 0, // 爱心捐赠是否成功
      showSharePop: false, // 爱心捐赠分享海报
      isShowWithoutPay: 0 // 是否展示 免密支付设置
    }
  },
  components: {
    AdBanner,
  ...
  beforeRouteLeave(to, from, next) {
    const toArr = ['order', 'payment', 'orderDetail', 'home', 'HuankeAccount', 'setTokenFreePayPwd']
  ...
  computed: {
    ...
    mlmBottomBtn() {
      ...
      if (this.isAppWxAvailabel || !this.wxApi.isweixin()) {
        // 原生换换商城App 微信登录已完全ready 或者 当前非微信环境
        txt = '下载换换App'
      }
      return txt
    },
    ifShowCouponWillRecieve() {
  ...
  methods: {
      ...
      getPayResult(this.orderSn)
        .then(
          res => {
            ...
            // 将获得优惠券提示
            this.coupon_count = res.coupon_count
            // 肺炎捐赠是否成功
            this.donated = res.isDonate

            // 是否要提醒设置免密支付
            this.isShowWithoutPay = res.isShowWithoutPay
    ...
    exportTime(orderTime) {
      ...
      return '请在' + minite + '分' + sec + '秒内完成现金支付,否则系统会自动取消订单，已支付的权益将自动退回您的账户'
    },
    phoneBack() {
      this.changeStatus(this.order_status_paid)
      this.$router.push({ name: 'order', params: { order: this.order_status_paid } })
