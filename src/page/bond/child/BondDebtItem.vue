<template>
  <div class="bond-item" :class="{ 'section-disabled': (!checkId && disabledCheckbox) || item.is_exchanging }">
    <div class="row row-1">
      ...
      <span class="is-exchanging" v-if="item.is_exchanging">兑换处理中</span>
    </div>
    <div class="row row-2">
      ...
      <div class="column column-right">
        <input
          ...
          :id="'id' + item.id"
          class="sel-debt"
          :disabled="(!checkId && disabledCheckbox) || !!item.is_exchanging"
          v-model="checkStatus"
        />
        <label class="sel-radius" placeholder="v" :for="'id' + item.id"></label>
      </div>
    </div>
    <label class="row row-3"
      >{{ utils.formatDate('YYYY-MM-DD', item.addtime) }}出借
      {{ utils.formatDate('YYYY-MM-DD', item.repayment_time) }}到期
    </label>
  </div>
</template>
