<template>
  <div class="container">
    <mt-header class="header" title="权益充值">
      <header-item slot="left" v-bind:isBack="true" v-on:onclick="goBack"></header-item>
    </mt-header>
    <div class="form-wrapper">
      <p class="tips">充值权益后，可在商城购买商品时，抵扣部分现金</p>
      <input
        type="text"
        maxLength="16"
        class="input"
      ...
      <router-link tag="p" class="help" :to="{ name: 'rechargeTips' }">权益充值卡密说明</router-link>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    errTimes() {
      return this.isShowErr && this.errMsg && this.timer && this.timer < 3
    }
  },
  ...
  methods: {
    ...
    recharge() {
      ...
      rechargeHB(params).then(
        res => {
          let msg = `<p class="recharge-success-alert">您的编号为<span>${res.huanCardId}</span>的卡密已经充值成功！本次充值权益<span>${res.suplus}</span>个</p><p class="recharge-success-alert">当前账户权益余额<span>${res.now_surplus}</span>个</p>`
          this.$messagebox({
            title: '充值成功',
            message: msg,
