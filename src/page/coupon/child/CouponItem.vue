<template>
  <div class="coupon-container" :class="{ 'trans-b': opacity }">
    <div class="coupon-info" :class="{ trans: opacity }">
      ...
      <div class="right">
        <p class="coupon-name">
          <label v-if="isSurplusCoupon"><span>店铺券</span></label
          ><label class="huan" v-else><span>换换券</span></label
          >{{ item.coupon_name }}
        </p>
...
export default {
  ...
  computed: {
    ...
    isSurplusCoupon() {
      // 是否为店铺优惠券
      return this.item.suppliers_id ? true : false
    }
  },
  methods: {
    btnClick() {
    ...
    .right {
      ...
      .coupon-name {
        ...
        label {
          width: 36px;
          height: 13px;
          border: 1px solid #ed690d;
          border-radius: 8px;
          margin-right: 4px;
          span {
            display: inline-block;
            @include sc(10px, #ed690d);
            line-height: 13px;
          }
          &.huan {
            border-color: #9b210b;
            span {
              @include sc(10px, #9b210b);
            }
          }
        }
      }
