...
const mutations = {
  ...
  saveCouponList(state, response) {
    // state.orginGoodsList = [...list]
    let coupon_list = []
    response.forEach(item => {
      let coupon = null
      if (item.coupon && item.coupon.has_coupon) coupon = item.coupon.coupon_list
      coupon_list.push(coupon)
    })
    state.couponList = [...coupon_list]
  },
  ...
  selectALL(state, isSelectAll) {
    ...
    if (isSelectAll) {
      state.goodsList.forEach((ele, index) => {
        // 结算 模式下，失效商品不可选择
        if (1 == state.mode && !state.goodsList[index].attr_stock) {
          return
        }
...
const actions = {
  ...
  fetchOrderPrice({ commit, getters }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      orderPrice(getters.choosenGoodsInfoForOrderPrice).then(
        res => {
          // 已选商品总价
