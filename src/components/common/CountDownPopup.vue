<template>
  <mt-popup class="mint-popup" v-model="showPopup" position="center" :close-on-click-modal="false">
    <div class="content">
  ...
  methods: {
    ...
    exportTime() {
      ...
      if (restTime > 0) {
        ...
        const minite = Math.floor((restTime - hour * 3600) / 60)
        const sec = restTime - hour * 3600 - minite * 60
        this.timeTxt = `您的订单在${
          hour ? hour + '小时' : ''
        }${minite}分钟${sec}秒内未完成支付将被取消，请尽快完成支付！`
      } else {
