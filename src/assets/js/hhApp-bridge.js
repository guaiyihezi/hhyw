...
var hhmall = (function() {
  ...
  var imps = {
    ...
    getAppVersion: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.getAppVersion()
      }
      return getDataFromIos('appVersion')
    } /*打开新的Webview页面, 新页面关闭与老页面没有影响*/, // url 传绝对路径
    openPage: function(url) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.openPage(url)
      }
    ...
    share: function(text, imageUrl, platform, flag, title, url, description) {
      ...
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.share(text, imageUrl, platform, flag, title, url, description || '')
      }
      _request('share', {
        text: text,
        platform: platform,
    ...
    getDeviceIMEI: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.getDeviceIMEI()
      }
    } /*获取设备AndroidID号，优先使用------Android 专属*/,
    getDeviceAndroidID: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.getDeviceAndroidID()
      }
    }, // 打开微信或其他App；例如打开微信appScheme="weixin://"
    openOtherApp: function(scheme) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.openOtherApp(scheme)
      }
      _request('openOtherApp', { scheme: scheme })
    }, // 复制网页内容到App
    copyText: function(text) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.copyText(text)
      }
      _request('copyText', { text: text })
    }, // 刷新当前页面
    refreshPage: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.refreshPage()
      }
      _request('refreshPage')
    }, // 获取网络状态,-1 未联网，1 4G，2 WIFI
    getNetType: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.getNetType()
      }
      _request('getNetType')
    }, // 播放视频 url:视频地址
    playVideo: function(url) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.playVideo(url)
      }
      _request('playVideo', { url: url })
    }, // 打开原生页面
    openAppPage: function(schema) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.openAppPage(schema)
      }
      _request('openAppPage', { schema: schema })
    }, // 去评分
    toScore: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.toScore()
      }
      _request('toScore')
    }, // 打开原生支付页面，type 1 表示 微信，2 支付宝 orderInfo 注意区分微信和支付宝订单格式
    openPay: function(type, orderInfo) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.openPay(type, orderInfo)
      }
    ...
    isWXAppInstalled: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.isWXAppInstalled()
      }
    ...
    handleNavigation: function(show) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.handleNavigation(show)
      }
    ...
    cartChanged: function(num) {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.cartChanged(num)
      }
    ...
    goBack: function() {
      if (window.hhmallJavaObj) {
        return window.hhmallJavaObj.goBack()
      }
      _request('goBack')
    }
  }
  for (var key in imps) {
    if (window.hhmallJavaObj && window.hhmallJavaObj[key]) {
      imps[key] = (function(mn) {
        return function() {
          var appVersion = window.hhmallJavaObj.getAppVersion()
          ...
          if (mn == 'showUmengShare' && appVersion < 231) {
            ...
            return window.hhmallJavaObj[mn].apply(window.hhmallJavaObj, args)
          }
        }
      })(key)
    }
